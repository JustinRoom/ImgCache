...
import jsc.org.lib.img.crop.animation.SimpleValueAnimator;
import jsc.org.lib.img.crop.animation.SimpleValueAnimatorListener;
import jsc.org.lib.img.crop.animation.ValueAnimatorV14;
import jsc.org.lib.img.crop.animation.ValueAnimatorV8;
import jsc.org.lib.img.crop.callback.Callback;
import jsc.org.lib.img.crop.callback.CropCallback;
import jsc.org.lib.img.crop.callback.LoadCallback;
import jsc.org.lib.img.crop.callback.SaveCallback;
...
public class CropImageView extends androidx.appcompat.widget.AppCompatImageView {
    ...
    private final Paint mPaintTranslucent = new Paint(Paint.ANTI_ALIAS_FLAG);
    private final Paint mPaintFrame = new Paint(Paint.ANTI_ALIAS_FLAG);
    private final Paint mPaintBitmap = new Paint(Paint.ANTI_ALIAS_FLAG);
    private final Paint mPaintDebug = new Paint(Paint.ANTI_ALIAS_FLAG);
    ...
    private final AtomicBoolean mIsCropping = new AtomicBoolean(false);
    ...
    private void handleStyleable(Context context, AttributeSet attrs, int defStyle, float mDensity) {
        ...
        try {
            drawable = ta.getDrawable(R.styleable.CropImageView_civ_img_src);
            if (drawable != null) setImageDrawable(drawable);
            for (CropMode mode : CropMode.values()) {
                if (ta.getInt(R.styleable.CropImageView_civ_crop_mode, 3) == mode.getId()) {
                    mCropMode = mode;
                    break;
                }
            }
            mBackgroundColor =
                    ta.getColor(R.styleable.CropImageView_civ_background_color, TRANSPARENT);
            mOverlayColor =
                    ta.getColor(R.styleable.CropImageView_civ_overlay_color, TRANSLUCENT_BLACK);
            mFrameColor = ta.getColor(R.styleable.CropImageView_civ_frame_color, WHITE);
            mHandleColor = ta.getColor(R.styleable.CropImageView_civ_handle_color, WHITE);
            mGuideColor = ta.getColor(R.styleable.CropImageView_civ_guide_color, TRANSLUCENT_WHITE);
            for (ShowMode mode : ShowMode.values()) {
                if (ta.getInt(R.styleable.CropImageView_civ_guide_show_mode, 1) == mode.getId()) {
                    mGuideShowMode = mode;
                    break;
                }
            }
            ...
            for (ShowMode mode : ShowMode.values()) {
                if (ta.getInt(R.styleable.CropImageView_civ_handle_show_mode, 1) == mode.getId()) {
            ...
            mHandleSize = ta.getDimensionPixelSize(R.styleable.CropImageView_civ_handle_size,
                    (int) (HANDLE_SIZE_IN_DP * mDensity));
            mTouchPadding = ta.getDimensionPixelSize(R.styleable.CropImageView_civ_touch_padding, 0);
            mMinFrameSize = ta.getDimensionPixelSize(R.styleable.CropImageView_civ_min_frame_size,
                    (int) (MIN_FRAME_SIZE_IN_DP * mDensity));
            mFrameStrokeWeight =
                    ta.getDimensionPixelSize(R.styleable.CropImageView_civ_frame_stroke_weight,
                            (int) (FRAME_STROKE_WEIGHT_IN_DP * mDensity));
            mGuideStrokeWeight =
                    ta.getDimensionPixelSize(R.styleable.CropImageView_civ_guide_stroke_weight,
                            (int) (GUIDE_STROKE_WEIGHT_IN_DP * mDensity));
            mIsCropEnabled = ta.getBoolean(R.styleable.CropImageView_civ_crop_enabled, true);
            mInitialFrameScale = constrain(
                    ta.getFloat(R.styleable.CropImageView_civ_initial_frame_scale,
                            DEFAULT_INITIAL_FRAME_SCALE), 0.01f, 1.0f, DEFAULT_INITIAL_FRAME_SCALE);
            mIsAnimationEnabled =
                    ta.getBoolean(R.styleable.CropImageView_civ_animation_enabled, true);
            mAnimationDurationMillis = ta.getInt(R.styleable.CropImageView_civ_animation_duration,
                    DEFAULT_ANIMATION_DURATION_MILLIS);
            mIsHandleShadowEnabled =
                    ta.getBoolean(R.styleable.CropImageView_civ_handle_shadow_enabled, true);
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            ta.recycle();
    ...
    private Bitmap scaleBitmapIfNeeded(Bitmap cropped) {
        ...
        if (outWidth > 0 && outHeight > 0) {
            Bitmap scaled = ImgUtils.getScaledBitmap(cropped, outWidth, outHeight);
    ...
    private Uri saveImage(Bitmap bitmap, final Uri uri) throws IOException, IllegalStateException {
        ...
        try {
            ...
            ImgUtils.copyExifInfo(getContext(), mSourceUri, uri, bitmap.getWidth(), bitmap.getHeight());
            ImgUtils.updateGalleryInfo(getContext(), uri);
            return uri;
        } finally {
    ...
    private Bitmap getImage(Uri sourceUri) {
        ...
        mExifRotation = ImgUtils.getExifOrientation(getContext(), mSourceUri);
        int maxSize = ImgUtils.getMaxSize();
        int requestSize = Math.max(mViewWidth, mViewHeight);
        if (requestSize == 0) requestSize = maxSize;

        final Bitmap sampledBitmap = ImgUtils.decodeSampledBitmapFromUri(getContext(), mSourceUri, requestSize);
        mInputImageWidth = ImgUtils.sInputImageWidth;
        mInputImageHeight = ImgUtils.sInputImageHeight;
